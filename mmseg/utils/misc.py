...
def target_stack_batch(inputs: List[torch.Tensor],
                data_samples: Optional[SampleList] = None,
                size: Optional[tuple] = None,
                size_divisor: Optional[int] = None,
                pad_val: Union[int, float] = 0,
...
def dual_stack_batch(inputs: List[torch.Tensor],
                     targets: Optional[SampleList] = None,
                data_samples: Optional[SampleList] = None,
                size: Optional[tuple] = None,
                size_divisor: Optional[int] = None,
                pad_val: Union[int, float] = 0,
    ...
    Args:
        inputs (List[Tensor]): The input multiple tensors. each is a
            CHW 3D-tensor.
        targets (List[Tensor]): The targets multiple tensors. each is a
            CHW 3D-tensor.
        data_samples (list[:obj:`SegDataSample`]): The list of data samples.
            It usually includes information such as `gt_sem_seg`.
        size (tuple, optional): Fixed padding size.
    ...
    max_size = np.stack(inputs_sizes).max(0)
    t_max_size = np.stack(target_sizes).max(0)
    max_size = np.stack([max_size,t_max_size]).max(0)
    if size_divisor is not None and size_divisor > 1:
        # the last two dims are H,W, both subject to divisibility requirement
        max_size = (max_size +
                    (size_divisor - 1)) // size_divisor * size_divisor
    ...
    for i in range(len(inputs)):
        ...
        if size is not None:
            ...
            t_width = max(size[-1] - t_tensor.shape[-1], 0)
            t_height = max(size[-2] - t_tensor.shape[-2], 0)
            # (padding_left, padding_right, padding_top, padding_bottom)
            t_padding_size = (0, t_width, 0, t_height)
        elif size_divisor is not None:
            width = max(max_size[-1] - tensor.shape[-1], 0)
            height = max(max_size[-2] - tensor.shape[-2], 0)
            padding_size = (0, width, 0, height)

            t_width = max(max_size[-1] - t_tensor.shape[-1], 0)
            t_height = max(max_size[-2] - t_tensor.shape[-2], 0)
            t_padding_size = (0, t_width, 0, t_height)
        else:
            padding_size = [0, 0, 0, 0]
            t_padding_size = [0, 0, 0, 0]
